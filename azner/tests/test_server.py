...
from azner.tests.utils import get_TransformersModelForTokenClassificationNerStep_model_path, SKIP_MESSAGE
...
def test_api():
    ...
    with initialize_config_dir(config_dir=os.environ.get("KAZU_TEST_CONFIG_DIR")):
        ...
        response = requests.post(
            f"http://127.0.0.1:{cfg.ray.serve.port}/api/{AZNER}/", json={"text": "Why do we always test EGFR in "
                                                                                 "these applications?"}
        ).json()
