...
class SeqTagProcessor(DataProcessor):
    ...
    def convert_examples_to_features(
        ...
        assert (
            output_mode == "seqtag"
        ), "Wrong processor! Processors for sequence classification tasks must inherits SeqClsProcessor"

        return self.sequence_tagging_task_convert_examples_to_features(
            examples, label_list, max_seq_length, tokenizer, output_dir, mode
        )  # mode : train, eval, or test
    ...
    def convert_single_example(
        self, ex_index, example, label_map, max_seq_length, tokenizer, mode
    ):  # For NER only
