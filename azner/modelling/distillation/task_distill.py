...
from azner.modelling.distillation.distillation.DistillModel import (
    SequenceTaggingTaskSpecificDistillation,
)
...
def __get_logger():
    ...
    logging.basicConfig(
        stream=sys.stdout, level=logging.INFO, format=log_format, datefmt="%m/%d/%Y %H:%M:%S"
    )
...
def main():
    ...
    if not args.aug_train:
        train_dataloader, len_train_examples = get_data_loader(
            args,
            data_type="train",
            processor=processor,
    ...
    eval_dataloader, _ = get_data_loader(
        args,
    ...
    num_train_optimization_steps = (
        int(len_train_examples / args.train_batch_size / args.gradient_accumulation_steps)
        * args.num_train_epochs
    )
    args.warmup_steps = (
        int(args.warmup_proportion * num_train_optimization_steps)
    ...
    if args.early_stop:
        earlystop_callback = EarlyStopping(
            ...
            mode=monitor_mode,
            min_delta=0.00,
            patience=5 if args.save_every_n_steps is not None else 3,  # TODO
            verbose=True,
        )
    ...
    trainer = pl.Trainer(
        ...
        val_check_interval=args.save_every_n_steps if args.save_every_n_steps is not None else 1.0,
        accumulate_grad_batches=args.gradient_accumulation_steps,
        devices=devices_count,
        accelerator=device_type,
        max_epochs=args.num_train_epochs,
        max_steps=args.num_train_steps,  # whichever reaches earlier
    )
