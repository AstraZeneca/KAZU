...
from azner.modelling.distillation.dataprocessor.seqcls_tasks import *
from azner.modelling.distillation.dataprocessor.seqtag_tasks import *
...
def argparse_get_args():
    ...
    parser.add_argument("--early_stop",
                        action='store_true',
                        help="Whether to use early_stop")
...
def args_sanity_check(args):
    ...
    if args.save_every_n_steps != None:
        assert args.save_every_n_epochs == None, "One of args.save_every_n_steps or args.save_every_n_epochs should be None: %s, %s"%(args.save_every_n_steps, args.save_every_n_epochs)
        assert float(args.save_every_n_steps).is_integer(), "args.save_every_n_steps should be integer"
    elif args.save_every_n_epochs != None:
        assert float(args.save_every_n_epochs).is_integer(), "args.save_every_n_epochs should be integer"
    else:
        args.save_every_n_epochs = 1
    logger.info('Save frequency: args.save_every_n_steps: {}, args.save_every_n_epochs: {}'.format(args.save_every_n_steps, args.save_every_n_epochs))
...
def set_accelerator(args):
    ...
    """
    if args.gpus != None and args.gpus > 0:
    ...
    logger.info("device: {}, avaliable gpus: {}, # of gpus used: {}".format(device_type, n_gpu, devices_count))
...
def additional_task_params(args):
    ...
    if not args.pred_distill and not args.do_eval:
        if args.task_name in default_params:
            ...
            logger.info("Overriding args.num_train_epoch as: {} (preset)".format(args.num_train_epoch))

    if args.task_type == None and args.task_name in default_params:
    ...
    if args.metric == None or args.metric not in ["entity_f1", "accuracy"]:
...
def set_metric_options(metric=None):
    ...
    """
    if metric == "entity_f1":
        ckpt_filename = "student_model-{epoch:02d}-{valF1:.4f}-{validation_loss_epoch:.3f}-{step:05d}" 
