    ...
    def save_checkpoint(
        ...
        if os.path.basename(path) != "last.ckpt":
            ...
            with open(output_log_file, "a") as callbackLogFP:
                jsonable_callback_log = {k:str(v) for k,v in checkpoint["callbacks"][list(checkpoint["callbacks"].keys())[0]].items()}
                json.dump(obj=jsonable_callback_log, fp=callbackLogFP)
                callbackLogFP.write("\n")
        
        studentModel_state_dict = {key[len("student_model."):]:value for key, value in checkpoint["state_dict"].items() if key.startswith("student_model.")}
        teacherModel_state_dict = {key:value for key, value in checkpoint["state_dict"].items() if key.startswith("teacher_model.")}
        assert len(checkpoint["state_dict"]) == len(studentModel_state_dict) + len(teacherModel_state_dict), "Missing structures while saving trained model."
