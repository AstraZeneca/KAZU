    ...
    def training_step(self, batch, batch_idx):
        ...
        if not self.args.pred_distill:  # Intermediate-layer (Transformer, Embedding) Distillation
            ...
            loss = self.soft_cross_entropy(
                student_logits / self.args.temperature, teacher_logits / self.args.temperature
            )
...
class SequenceTaggingTaskSpecificDistillation(TaskSpecificDistillation):
    def __init__(
        ...
        **config_kwargs,
        ...
        super().__init__(
            args=args, schedule=schedule, num_train_optimization_steps=num_train_optimization_steps
        )
        ...
        self.teacher_model = TinyBertForSequenceTagging.from_pretrained(
            args.teacher_model, num_labels=self.num_labels
        )
        self.student_model = TinyBertForSequenceTagging.from_pretrained(
            args.student_model, num_labels=self.num_labels
        )
