...
class StringPreprocessorStep(BaseStep):
    """
    String Preprocessing steps (subclass of :class:`azner.steps.string_preprocessing.StringPreprocessorStep` are a
    ...
    Implementations will modify the return value of :meth:`azner.data.data.Section.get_text`. The
    idea is that implementations of these will perform any required string preprocessing before the section text is
    passed to another :class:`azner.steps.base.step.Step`.  Implementations of this step update the
    :attr:`azner.data.data.Section.offset_map`, so that offsets for objects generated by subsequent steps can be
