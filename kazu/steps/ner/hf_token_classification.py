...
class TransformersModelForTokenClassificationNerStep(BaseStep):
    ...
    def __init__(
        ...
        self.activation_fn = Sigmoid() if detect_subspans else Softmax(dim=-1)
        self.tokenized_word_processor = TokenizedWordProcessor(
            detect_subspans=detect_subspans,
            confidence_threshold=threshold,
            id2label=TransformerUtils.id2labels_from_label_list(labels),
        )
...
class TransformerUtils:
    ...
    def frame_to_tok_word(
        ...
        :param predictions:
        :param tokenizer: tokenizer used to generate the BatchEncoding
        :param stride: stride length used by tokenizer for overflowing inputs
        ...
        logger.debug(
            f"inputs this frame: {tokenizer.decode(frame_token_ids[:start_index])}<-IGNORED "
            f"START->{tokenizer.decode(frame_token_ids[start_index:end_index])}"
            f"<-END IGNORED->{tokenizer.decode(frame_token_ids[end_index:])}"
        )
        return all_words
    ...
    def section_frames_to_tokenised_words(
        ...
        section_frame_indices = TransformerUtils.get_list_of_batch_encoding_frames_for_section(
            batch_encoding, section_index
        )
        for frame_index, section_frame_index in enumerate(section_frame_indices):
            word_sub_list = TransformerUtils.frame_to_tok_word(
    ...
    @staticmethod
    def id2labels_from_label_list(labels: List[str]) -> Dict[int, str]:
