...
def test_enumerate_dataframe_chunks():
    ...
    for chunk_size in range(1, len(DummyParser.DUMMY_DATA[IDX]) + 1):
...
def test_embedding_cache_manager(tmp_path: Path, index_type: str):
    in_path = tmp_path.joinpath(DummyParser.DUMMY_SOURCE)
    in_path.mkdir()
    ...
    assert len(index) == len(set(DummyParser.DUMMY_DATA[IDX]))
...
def test_cached_index_group(tmp_path: Path):
    in_path1 = tmp_path.joinpath(DummyParser.DUMMY_SOURCE + "1")
    in_path1.mkdir()

    parser1 = DummyParser(in_path=str(in_path1))
    parser1.name = "ontology_1"
    in_path2 = tmp_path.joinpath(DummyParser.DUMMY_SOURCE + "2")
    in_path2.mkdir()
